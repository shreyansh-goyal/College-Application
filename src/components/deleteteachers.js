...
class DeleteTeachers extends Component {
  deleteTeacher = () => {
    axios
      .get(
        "https://bpitconnect.herokuapp.com/teachers?teacherId=" +
          this.refs.teacherId.value
      )
      .then(data => {
        console.log(data.data);
        axios
          .delete(
            "https://bpitconnect.herokuapp.com/teachers/" + data.data[0].id
          )
          .then(data => {
  ...
  render() {
    return (
      <div>
        ...
        >
          <button className="btn btn-info" onClick={this.deleteTeacher}>
            Delete Teacher
          </button>
        </div>
      </div>
